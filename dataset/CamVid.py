...
class CamVid(torch.utils.data.Dataset):
    def __init__(self, dir_paths, label_path, csv_path, scale, loss='dice', mode='train'):
        ...
        for dir_paths_ in dir_paths:
            dir_list = [x[0] for x in os.walk(dir_paths_)]
            for dir_ in dir_list:
                if os.path.isfile(os.path.join(dir_, 'forged_image.jpg')):
                    if os.path.isfile(os.path.join(dir_, 'target_mask.jpg')):
                        self.image_list.append(os.path.join(dir_, 'forged_image.jpg'))
                        self.label_list.append(os.path.join(dir_, 'target_mask.jpg'))
    ...
    def __getitem__(self, index):
        ...
        img = np.array(img)
        # load label
        source_label_filename = self.label_list[index]
        source_label_filename = source_label_filename.replace('target_mask.jpg', 'source_mask.jpg')
        source_label = Image.open(source_label_filename)
        source_label = source_label.convert('RGB')


        label = Image.open(self.label_list[index])
        label = label.convert('RGB')

        cv2_img1 = np.array(label)
        cv2_img2 = np.array(source_label)

        cv2_img = cv2.add(cv2_img1, cv2_img2)

        label =  Image.fromarray(cv2_img)

        # cv2.imshow('1', cv2_img1)
        # cv2.imshow('2', cv2_img2)
        # cv2.imshow('3', cv2_img)
        ...
        if self.mode == 'test':
            resize = iaa.Scale({'height': 960, 'width': 1280})
